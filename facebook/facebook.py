...
import warnings
from facebook.misc_date import convert_date, strip_date, get_time


class FaceBook:
    def __init__(self, output_path: str = "output.csv"):
        self.write_to_csv(output_path)
        self.set_df(output_path)
        self.set_data()

    def load_messages(self, writer, messages_path: str = "messages"):
        directory = os.path.join(os.path.dirname(os.path.dirname(__file__)), messages_path)
            ...
            for filename in os.listdir(absolute_directory):
                if filename.endswith(".json"):
                    ...
                    for message in data["messages"]:
                        message_date = datetime.fromtimestamp(message.get("timestamp_ms", 0) / 1000).strftime(
                            "%Y-%m-%d %H:%M:%S")
                        ...
                        writer.writerow(
                            [message_date, sender, message_type, normalized_content, photos, videos, call_duration])
    ...
    def write_to_csv(self, output_path):
        ...
        if not os.path.isfile(output_path):
            with open(output_path, "a") as csv_file:
                ...
                self.load_messages(writer)

    def set_df(self, input_path):
    ...
    def set_data(self):
        ...
        self.series_message = self.date_group_by.apply(
            lambda x: len(x[(x["Type"] == "Generic") | (x["Type"] == "Share")]))
    ...
    def plot_videocall_graph(self):
        ...
        fig.update_layout(title="Video Call Duration Per Day", xaxis_title="Date",
                          yaxis_title="Duration of Calls(hours)")
        fig.update_layout({'plot_bgcolor': 'rgba(0,0,0,0)', 'paper_bgcolor': 'rgba(0,0,0,0)'})
        fig.show()
    ...
    def plot_24hour_graph(self):
        ...
        fig.update_layout(title="Average Messages Per 24hours", xaxis_title="Time",
                          yaxis_title="Average Number of Messages")
        fig.update_layout({'plot_bgcolor': 'rgba(0,0,0,0)', 'paper_bgcolor': 'rgba(0,0,0,0)'})
        fig.show()
    ...
    def plot_senders_pie_chart(self):
        ...
        for i, _ in enumerate(self.sender_name_list):
            ...
            sender_name_plain_text = self.sender_groupby.apply(
                lambda x: len(x[(x["Type"] == "Generic") & (x["Photos"] == 0) & (x["Videos"] == 0)]))[i]
    ...
    def plot_distribution_pie_chart(self):
        ...
        pie_chart_values = [self.number_of_photos, self.number_of_texts, self.number_of_attachments,
